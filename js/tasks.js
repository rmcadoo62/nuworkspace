  ...
  const taskRows = allItems.map(entry => {
    ...
    return `
      <div class="itt-row" data-task-id="${t._id}" draggable="true" style="${t.status==='billed'?'background:rgba(192,132,252,0.50);border-color:rgba(192,132,252,0.70);':t.status==='cancelled'?'background:rgba(232,162,52,0.50);border-color:rgba(232,162,52,0.70);border-left:3px solid #e8a234;':t.status==='complete'||t.done?'background:rgba(120,120,130,0.50);border-color:rgba(120,120,130,0.70);':t.status==='inprogress'?'background:rgba(46,158,98,0.50);border-color:rgba(46,158,98,0.70);':''}${_stripeCss}"
        ...
        <div class="itt-price ${canEditBilled?'itt-cell-edit':''}" ${canEditBilled?`onclick="inlineEditPrice('${t._id}','${projId}');event.stopPropagation()"`:''}  style="font-family:'JetBrains Mono',monospace;font-size:12px;color:${(t.status==='cancelled' || t.fixedPrice<0) ? 'var(--red)' : 'var(--green)'};font-weight:700;${canEditBilled?'cursor:text':''}"${billedLock ? ' title="Billed — locked. Unlock from the Billing Queue."' : ''}>
          ${t.fixedPrice ? (t.status==='cancelled' ? '($'+Math.abs(t.fixedPrice).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+')' : t.fixedPrice<0 ? '($'+Math.abs(t.fixedPrice).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+') <span style="font-size:9px;font-weight:700;margin-left:2px">CR</span>' : '$'+t.fixedPrice.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})) : '—'}
        </div>
