...
const ASSESSMENT_EVIDENCE = {
  ...
  'SC.L2-3.13.10':'BitLocker recovery keys stored securely in Active Directory accessible only to Owner/IT Administrator; NUEncrypted shared folder encryption key stored in Synology Key Manager accessible only via passphrase known exclusively to Owner/IT Administrator; no key files distributed to employees; keys not stored in plaintext or in locations accessible to standard users; key inventory documents all encrypted systems and key storage locations; recovery key access logged.',
  ...
  'SC.L2-3.13.16':'All Windows CUI endpoints have BitLocker AES-256 full-disk encryption enabled via Group Policy; CUI stored on Synology NAS in AES-256 encrypted shared folder (NUEncrypted) automatically mounted Mon–Fri at start of business and unmounted at close of business via Task Scheduler; folder does not auto-mount on reboot — manual Owner/IT Administrator intervention required; no key files distributed to employees; no CUI permitted on unencrypted media, unencrypted cloud storage, or personal devices; encryption verified active quarterly.',
