...
function renderDashboard() {
  ...
  // Build yearly booking data: bookings[mo][cat] and reversals[mo][cat]
  ...
  taskStore.forEach(t => {
    ...
    // Positive booking — created this year, not cancelled
    const createdMo = (t.createdAt||'').slice(0,7);
    if (createdMo.slice(0,4) === String(_curYear) && t.status !== 'cancelled') {
      const mo = parseInt(createdMo.slice(5,7)) - 1;
      if (mo >= 0 && mo <= _curMonth) {
        if (!_yearBookings[mo][cat]) _yearBookings[mo][cat] = 0;
        _yearBookings[mo][cat] += (t.fixedPrice || 0);
      }
    }
    // Reversal — cancelled this year
    const cancelledMo = (t.cancelledDate||'').slice(0,7);
    if (cancelledMo && cancelledMo.slice(0,4) === String(_curYear)) {
      const mo = parseInt(cancelledMo.slice(5,7)) - 1;
      if (mo >= 0 && mo <= _curMonth) {
        if (!_yearReversals[mo][cat]) _yearReversals[mo][cat] = 0;
        _yearReversals[mo][cat] += (t.fixedPrice || 0);
      }
    }
  });
  ...
  // Current-month detail tasks (for drill-down)
  ...
  function buildYearGrid() {
    ...
    const bodyRows = _allCats.map((cat, ci) => {
      ...
      const cells = _monthNames.slice(0, _curMonth+1).map((m, mi) => {
        const book = _yearBookings[mi]?.[cat] || 0;
        const rev  = _yearReversals[mi]?.[cat] || 0;
        const net  = book - rev;
        colNetTotals[mi] += net;
        ytdCat += net;
      ...
      ytdGrand += ytdCat;
      const rowBg = ci%2===1 ? 'background:var(--surface2)' : '';
    ...
    const totalCells = colNetTotals.map((v, mi) => {
      const isCur = mi === _curMonth;
      const bg = isCur ? 'background:rgba(232,162,52,0.10);' : 'background:var(--surface2);';
      return `<td style="text-align:right;padding:7px 10px;font-weight:700;${bg}">${fmtNetCell(v)}</td>`;
    }).join('');
  ...
  // Build month detail HTML (for a given month)
  function buildMonthDetail(mo) {
    const moKey = _curYear + '-' + String(mo+1).padStart(2,'0');
    const moLabel = _monthNames[mo] + ' ' + _curYear;
    const cats = [...new Set([
      ...Object.keys(_yearBookings[mo]||{}),
      ...Object.keys(_yearReversals[mo]||{}),
    ])].sort();
    if (!cats.length) return `<div style="text-align:center;padding:24px;color:var(--muted);font-size:13px">No bookings in ${moLabel}.</div>`;
    ...
    const rows = cats.map((cat, ci) => {
      ...
      const bg = ci%2===1 ? 'background:var(--surface2)' : '';
      return `<tr style="border-bottom:0.5px solid var(--border);${bg}">
  ...
  // Build detail task drill-down for a given month
  function buildMonthTaskDetail(mo) {
    ...
    return '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:12px"><thead><tr style="border-bottom:2px solid var(--border)">'+
      ...
      Object.keys(byCat).sort().map(cat => {
        ...
        return '<tr style="background:var(--surface2);border-bottom:0.5px solid var(--border)">'+
          ...
          '<td style="padding:7px 10px;text-align:right;font-weight:700;color:'+(catNet<0?'var(--red)':'var(--amber)')+'">'+
            (catNet<0 ? '($'+Math.abs(catNet).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+')' : '$'+catNet.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}))+
          '</td></tr>'+
          catTasks.map(t => {
            const p = projects.find(x=>x.id===t.proj);
            const isRev = cancelledMo.has(t._id);
            const pColor = isRev ? 'var(--red)' : 'var(--green)';
            const pDisplay = t.fixedPrice>0 ? (isRev?'($'+t.fixedPrice.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+')':'$'+t.fixedPrice.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})) : '—';
            return '<tr style="border-bottom:0.5px solid var(--border)">'+
              '<td style="padding:5px 10px 5px 22px;color:'+(isRev?'var(--muted)':'var(--text)')+';font-size:11px">'+(isRev?'<span style="font-size:9px;font-weight:700;color:var(--red);margin-right:4px">↩ REVERSAL</span>':'')+t.name+'</td>'+
  ...
  // Expose month builders so setBkMonth can re-render without full dashboard refresh
...
function setBkView(view) {
  ...
  const activeStyle   = 'var(--amber-glow)';
  const inactiveStyle = 'transparent';
  btnYear.style.background  = isYear ? activeStyle : inactiveStyle;
  btnYear.style.color       = isYear ? 'var(--amber)' : 'var(--muted)';
  btnYear.style.borderColor = isYear ? 'var(--amber-dim)' : 'var(--border)';
  btnMonth.style.background  = !isYear ? activeStyle : inactiveStyle;
...
function setBkMonth(mo) {
  ...
  document.querySelectorAll('[id^="bkTab_"]').forEach(btn => {
    ...
    btn.style.borderColor = active ? 'var(--border)' : 'var(--border)';
  });
  // Re-render month content — re-invoke renderDashboard would be heavy,
  // so we call the global helpers stored on window during last render
